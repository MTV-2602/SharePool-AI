...
export default function ClientCheckPage() {
  ...
  // Guide tabs: 'codex' or 'antigravity'
  const [guideTab, setGuideTab] = useState("codex");
  
  const [copiedField, setCopiedField] = useState("");
  const [origin, setOrigin] = useState("http://localhost:20127");
  const router = useRouter();

  useEffect(() => {
    if (typeof window !== "undefined") {
      setOrigin(window.location.origin);
      const k = localStorage.getItem("clientKey");
      if (k) {
        setSavedKey(k);
        setKey(k);
        fetchKeyDetails(k);
      ...
      <div className="min-h-screen flex items-center justify-center bg-bg p-4 relative overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-text-muted">Nhập API Key để kiểm tra hạn mức & hướng dẫn kết nối</p>
          </div>

          <Card>
            <form onSubmit={handleLogin} className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium">Mã API Key</label>
          ...
          <div className="text-center mt-6">
            <button
              ...
              className="text-sm text-text-muted hover:text-primary transition-colors inline-flex items-center gap-1 cursor-pointer"
            >
              <span className="material-symbols-outlined text-[16px]">arrow_back</span>
              Quay lại trang quản trị (Admin)
            </button>
          </div>
        </div>
      </div>
      ...
      <header className="border-b border-border bg-surface/80 backdrop-blur-md sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-4 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => fetchKeyDetails(savedKey)}
              disabled={loading}
      ...
      <main className="max-w-6xl mx-auto px-4 mt-8">
        ...
        {activeTab === "usage" && (
          <div className="space-y-6">
            ...
            {/* Progress Bar (Only show if not infinite) */}
            {!isInfinite && (
              <Card>
...
supports_websockets = false`, "tomlConfig")}
            ...
            {guideTab === "antigravity" && (
              <div className="space-y-6">
                <Card title="🪐 Hướng dẫn kết nối đến AntiGravity Portal (Gemini)" icon="info">
                  <p className="text-sm text-text-muted mt-2">
                    VS Code Extension **Google Gemini Code Assist (Cloud Code)** có thể được định cấu hình chuyển hướng traffic qua 9Router Server để xoay vòng tài nguyên của bạn.
                  </p>
                </Card>
                ...
                <Card title="PHƯƠNG PHÁP A: Dành cho công cụ hỗ trợ Custom Base URL" icon="cloud">
                  <div className="space-y-3 text-sm text-text-muted mt-2">
                    <p>Nếu bạn dùng các công cụ lập trình hỗ trợ Custom Base URL (như Cursor, Cline, RooCode, Continue...):</p>
                ...
                <Card title="PHƯƠNG PHÁP B: Sử dụng Script Proxy Siêu Nhẹ (VS Code Extension)" icon="settings_ethernet">
                  <div className="space-y-4 text-sm text-text-muted mt-2">
                    ...
                    <p className="italic text-xs">
                      (Khi bạn tắt proxy bằng <code>Ctrl+C</code>, file <code>hosts</code> trên máy của bạn sẽ tự động được khôi phục nguyên vẹn.)
                    </p>
                  </div>
                </Card>
              </div>
