...
export async function POST(request, { params }) {
  ...
  if (isClientKey) {
    ...
    if (!authResult.valid) {
      return Response.json(
        { error: { message: authResult.error, code: 401 } },
  ...
  try {
    ...
    if (stream) {
      ...
      finalResponse = transformOpenAISSEToGeminiSSE(response, model);
    } else {
      // Convert OpenAI JSON response => Gemini GenerateContentResponse
      finalResponse = await convertOpenAIResponseToGemini(response, model);
    }

    if (isClientKey) {
      return await wrapResponseWithClientKeyLogging(finalResponse, authResult.keyData.id, model);
    }
