...
/**
 * Store a thought signature for a tool_call_id with optional sessionId namespace (RAM LRU cache).
...
/**
 * Retrieve a thought signature by tool_call_id from RAM cache.
...
export async function getGeminiThoughtSignature(toolCallId, sessionId = null, model = null) {
  return getGeminiThoughtSignatureSync(toolCallId, sessionId, model);
}

/**
 * Synchronous get from RAM cache (for sync translators).
 * `model` is the target model; signatures produced by another model family are ignored.
 */
